...
void AppLauncherPlugin::loadButton() {
    ...
    if(!ok) {
        ok = QFile::exists(path);    //do it this way to ensure the file existance check never runs for /net/ files
    }
    if(!ok) {
        emit RemovePlugin(this->ID());
        return;
    }
    ...
    if(path.endsWith(".desktop") && ok) {
        ...
        if(!ok) {
            ...
            if(ICONS->exists(file.icon)) {
                iconID = file.icon;
            }
            else if(ICONS->exists(file.icon.section("-",0,-2)) ) {
                iconID = file.icon.section("-",0,-2);    //some icons get very specific with "-" delimiters, look for a more generic icon if possible
            }
            else {
                iconID = "system-run";
            }
            ...
            if(!file.comment.isEmpty()) {
                button->setToolTip(file.comment);
            }
            txt = file.name;
            ...
            if(!file.actions.isEmpty()) {
                for(int i=0; i<file.actions.length(); i++) {
                    ...
                    if(ICONS->exists(file.actions[i].icon)) {
                        ICONS->loadIcon(tmp, file.actions[i].icon);
                    }
                    else {
        ...
        if(info.isDir()) {
            if(path.startsWith("/media/") || path.startsWith("/run/media/")) {
                ...
                iconame = "folder";    //button->setIcon( LXDG::findIcon("folder","") );
            }
    ...
    if(!iconID.isEmpty()) {
        if(ICONS->isLoaded(iconID)) {
            ...
            if(!ICONS->exists(iconID) && iconID.contains("/") ) {
                iconID = iconID.replace("/","-");    //quick mimetype->icon replacement just in case
            }
    ...
    if(button->fontMetrics().horizontalAdvance(txt) > (button->width()-OUTMARGIN) ) {
        ...
        if(txt.contains("\n")) {
            ...
            for(int i=0; i<txtL.length(); i++) {
                if(i>1) {
                    txtL.removeAt(i);    //Only take the first two lines
                    i--;
                }
                else {
                    txtL[i] = button->fontMetrics().elidedText(txtL[i], Qt::ElideRight, (button->width()-OUTMARGIN) );
                }
            }
            txt = txtL.join("\n");
        } else {
...
void AppLauncherPlugin::fileDelete() {
    ...
    if(QFileInfo(path).isDir()) {
        QProcess::startDetached("rm", QStringList() << "-r" << path);
    }
    else {
        QFile::remove(path);
    }
