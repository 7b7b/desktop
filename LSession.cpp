...
void LSession::CleanupSession() {
    ...
    if( playaudio ) {
        ...
        if(sfile.isEmpty() || !QFile::exists(sfile)) {
            sfile = LOS::LuminaShare()+"Logout.ogg";
        }
        playAudioFile(sfile);
    }
...
void LSession::launchStartupApps() {
    ...
    if(sessionsettings->value("PlayStartupAudio",true).toBool()) {
        ...
        if(sfile.isEmpty() || !QFile::exists(sfile)) {
            sfile = LOS::LuminaShare()+"Login.ogg";
        }
        playAudioFile(sfile);
    }
...
void LSession::playAudioFile(QString filepath) {
    ...
    if(DEBUG) {
        qDebug() << "Play Audio File";
    }
    if(mediaObj==0) {
        qDebug() << " - Initialize media player";
        mediaObj = new QMediaPlayer();
    }
    if(mediaObj !=0 ) {
        if(DEBUG) {
            qDebug() << " - starting playback:" << filepath;
        }
